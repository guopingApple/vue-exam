<template>
    <div style="margin-top:15px;">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm" :inline-message='blone'>
            ...
            <el-form-item label="指定查看二维码用户" required prop="domains">
                <template>
                    <el-form-item 
                        v-for="(domain, index) in ruleForm.domains"
                        :prop="'domains.' + index + '.value'"
                        :key="domain.key"
                        :rules='rules1' style="margin-top:10px;">
  ...
  export default {
    data() {
       var checkCode = (rule, value, callback) => {
        ...
        if (value === '') {
            callback(new Error('请输入员工微信'));
        } else {
            let reg=/^[A-Za-z0-9]+$/;
              if(!reg.test(value)){
                  callback(new Error('请输入正确的员工微信'));
              }
            //   if(arrs.length!=0){
            //       let str=arrs.join(",")+','
      ...
      return {
        ...
        //   ],
          ...
          signTimeType: [
            { required: true, message: '请选择签到时间类型'}
          ],
          coverPicUrl:[
